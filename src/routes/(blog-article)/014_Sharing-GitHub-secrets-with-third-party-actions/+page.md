---
slug: 014_Sharing-GitHub-secrets-with-third-party-actions
title: Sharing GitHub Secrets with Third Party Actions
...
For instance, MegaLinter could release a new minor version of their package with malicious code. They could update one of the embedded packages in the docker image and that package could contain malicious code to obtain secrets using the `GITHUB_TOKEN`. Since you normally use a major version for the action `megalinter/megalinter/flavors/documentation@v5` you would use the newer version on the next workflow execution.
...
3. Grant each workflow only the permissions it needs.
